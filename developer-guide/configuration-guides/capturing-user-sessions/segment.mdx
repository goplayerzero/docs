...
# Segment Integration Landing Page

## Overview
- **What this section is for**: Enable PlayerZero in your app using Segment — no code changes required.
- **Key benefits**:
  - Seamless integration in seconds.
  - Instantly forward all `track()` and `identify()` calls to PlayerZero.
  - Automatic session monitoring without needing to update your frontend.
- **Who this is for**: Developers and analysts already using [Segment](https://segment.com/) for event tracking and identity management.

## Basic Setup Guide

### Prerequisites
- Access to Segment’s **Destination catalog**
- Your PlayerZero **Project ID** (from Settings > Data Collection)
...
## 3. Available Integration Guides

- **[npm Installation](/docs/integrations/npm)**  
  Use the PlayerZero SDK directly with modern frontend frameworks.

- **[Script Tag Setup](/docs/integrations/script-tag)**  
  Install with a `<script>` tag if you don’t use Segment or a bundler.

- **[Domain Configuration](/docs/domain-controls)**  
  Control which data is collected and adjust feature toggles.
