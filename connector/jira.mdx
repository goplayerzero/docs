...
- **Transition issues** — Move issues through workflow states (e.g., To Do → In Progress → Done) by specifying the target status
...
- **Edit and delete comments** — Modify or remove existing comments on issues
...
- **Link issues** — Create relationships between issues (blocks, is blocked by, duplicates, relates to, clones)
- **Set custom fields** — Populate custom fields on issue creation or update, including project-specific fields discovered through field inspection
- **Create sub-tasks** — Create child issues or sub-tasks within epic hierarchies by specifying a parent issue key
...
- **Debug-to-ticket workflow** — Convert investigation findings into properly formatted Jira issues with custom fields
- **Cross-reference analysis** — When debugging, agents search related Jira tickets for historical context and known issues
- **Workflow automation** — Automatically transition issues through workflow states as part of a multi-stage workflow (e.g., move to In Progress after triage, close after resolution)
- **Issue linking** — Connect related issues discovered during investigation (e.g., link a new bug to a known parent epic)
